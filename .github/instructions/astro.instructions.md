...
- Define a `Props` interface in the frontmatter of every component
- Document reusable component `Props` interfaces with TSDoc comments,
  especially for non-obvious props, defaults, and rendering constraints
...
- Explain why a component uses a non-obvious structure, accessibility pattern,
  or client-side script; do not narrate markup that is already clear.
- Keep comments focused on intent and decisions, and update or remove stale
  comments whenever the related component changes.
- Use TSDoc comments for reusable component props when their meaning is not
  obvious from the property name and type.
